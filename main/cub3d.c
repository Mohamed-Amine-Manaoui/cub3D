...
void my_mlx_pixel_put(t_mlx *mlx, int x, int y, int color) {
    if (x >= 0 && x < mlx->w_h_map[0] * 48 && y >= 0 && y < mlx->w_h_map[1] * 48) {
        char *dst;
...
void paint_square(t_mlx *mlx, int col, int row, char c) {
    ...
    while (i < SQUARE_SIZE) {
        j = 0;
        while (j < SQUARE_SIZE) {
            my_mlx_pixel_put(mlx, row * SQUARE_SIZE + j, col * SQUARE_SIZE + i, color);
            j++;
        }
        i++;
    }
    if (symbolic_character(c)) {
        draw_cercle(mlx, mlx->player_x_pos + SQUARE_SIZE / 2, mlx->player_y_pos + SQUARE_SIZE / 2, SQUARE_SIZE / 4);
    }
}
...
int main(int ac, char **av) 
{
    ...
    calcul_with_height(&mlx, &caste);
    mlx.ptr = mlx_init();
    mlx.ptr_win = mlx_new_window(mlx.ptr, mlx.w_h_map[0] * 48, mlx.w_h_map[1] * 48, "siri ghiyriha !!");
